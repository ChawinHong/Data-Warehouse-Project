...
BEGIN
	...
	SELECT 
		...
		,prd_nm
		,COALESCE(prd_cost,0) AS prd_cost -- Replace null values
		-- ,prd_line
		...
		,CAST(LEAD(prd_start_dt,1) OVER(PARTITION BY prd_key ORDER BY prd_start_dt ASC)-1 AS DATE) AS prd_end_dt -- Calculate end date as one day before the next start date
	...
	SELECT
		...
		CASE WHEN TRIM(cntry) IN ('DE') THEN 'Germany'
			...
			ELSE TRIM(cntry) END AS cntry -- Standardize and handle missing or blank country codes
